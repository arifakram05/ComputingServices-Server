...
public class SecureLogin {
	 ...
	 * 
	 * @param userId
	 *            user to whom token needs to be served
	 * @param loginDuration
	 ...
	 */
	public static String createJWT(Long userId, long loginDuration) throws NoSuchAlgorithmException {
	...
	/**
	 * Verify if the given token and user ID are related and valid
	 * 
	 * @param token
	 *            JWT token provided by the user to validate
	 * @param userIdToVerify
	 *            user trying to perform operation
	 * @return <i>true</i> if given user ID is valid
	 */
	public static boolean isTokenValid(String token, String userIdToVerify) {
		String knownUserId = Jwts.parser().setSigningKey(encodedSecretKey).parseClaimsJws(token).getBody().getId();
		LOGGER.info("Verifying token for user " + userIdToVerify);
		return userIdToVerify.equals(knownUserId);
	}
